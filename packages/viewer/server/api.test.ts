...
const FIXTURE = path.join(REPO_ROOT, 'packages/world-agent/viewer/fixtures/sample-run');
/** Last seq in the fixture's event log — the fixture is regenerated, so never hard-code it. */
const LAST_SEQ = readFileSync(path.join(FIXTURE, 'events.jsonl'), 'utf8').trim().split('\n').length;
let root: string;
...
before(() => {
  ...
  cpSync(FIXTURE, path.join(root, `packages/world-agent/runs/sample/${ID}`), { recursive: true });
  // A draft must be skipped; a broken run folder must not take the list down.
  writeFileSync(
    path.join(root, 'packages/interview-agent/worlds/x.draft.json'),
    '{ "slug": "x-draft"',
  );
  mkdirSync(path.join(root, 'packages/world-agent/runs/broken/20260101-000000'), {
    recursive: true,
  });
  writeFileSync(
    path.join(root, 'packages/world-agent/runs/broken/20260101-000000/run.json'),
    '{ not json',
  );
});
...
test('a path escape is refused', () => {
  for (const p of [
    '/api/runs/..%2F..%2Fetc/passwd',
    '/api/runs/sample/..',
    '/api/worlds/..%2Fworlds',
    '/runs/../../package.json/x/courtroom.html',
  ]) {
    const r = get(p);
...
test('/court returns RunData for the complete fixture', () => {
  ...
  assert.equal(d.seq, LAST_SEQ);
  assert.ok(d.truth, 'the fixture is complete, so the truth is in');
  // Schema v2: the trial is derived, the gates were raised, the bench advised.
  assert.ok(d.trial.verdict.options.length >= 2);
  assert.ok(d.trial.phases.length > 0);
  const g1 = d.gates.find((g) => g.id === 'G-01')!;
  assert.equal(g1.raisedBy.kind, 'challenge');
  assert.ok(g1.recommendation && g1.recommendationReason, 'the fixture bench advised on G-01');
});
...
test('/decide refuses a bad body and refuses when nothing is pending, before running anything', async () => {
  ...
  assert.equal(
    (await post(
      `/api/runs/sample/${ID}/verdict`,
      { optionId: 'resp_optimus', confidence: 101 },
      root,
    ))!.status,
    400,
  );
  assert.equal(
    (await post(`/api/runs/sample/${ID}/verdict`, { optionId: 'resp_optimus' }, root))!.status,
    409,
  );
  assert.equal(
    (await post('/api/runs/sample/nope/decide', { gateId: 'G-01', optionId: 'accept' }, root))!
      .status,
    404,
  );
...
test('/decide runs decide.ts when the gate is pending', async () => {
  // A real run: boot the Mike example under the temp root, then raise G-01 with
  // hand-written actions through propose.ts — a challenge to E-02 once it is in.
  ...
  const say = (id: string, over: Record<string, unknown>) =>
    harness(
      ['harness/propose.ts', dir, id, '-'],
      JSON.stringify({
        publicMessage: 'The record will show what it shows.',
        rationaleSummary: 't',
        action: 'speak',
        ...over,
      }),
  ...
  turn('DETECTIVE');
  const ch = say('DETECTIVE', { action: 'challenge_evidence', targetId: 'E-02' });
  assert.equal(ch.accepted, true, JSON.stringify(ch));
  ...
  const raised = court.gates.find((g) => g.id === 'G-01')!;
  assert.equal(raised.raisedBy.kind, 'challenge');
  assert.equal(raised.raisedBy.characterId, 'DETECTIVE');
  assert.equal(raised.recommendation, null, 'the bench has not spoken');

  const wrong = (await post(
    `/api/runs/${slug}/${id}/decide`,
    { gateId: 'G-02', optionId: 'examine_cookie' },
    root,
  ))!;
  assert.equal(wrong.status, 409);
  const bad = (await post(
    `/api/runs/${slug}/${id}/decide`,
    { gateId: 'G-01', optionId: 'no_such_option' },
    root,
  ))!;
  assert.equal(bad.status, 409, 'decide.ts rejected it');
  assert.match((bad.body as { error: string }).error, /no option/);

  // The bench advises through recommend.ts; the next /court carries it — what the stream-driven refetch shows.
  harness([
    'harness/recommend.ts',
    dir,
    'G-01',
    '--option',
    'admit_limited',
  ...
  const advised = body<RunData>(`/api/runs/${slug}/${id}/court`).gates.find(
    (g) => g.id === 'G-01',
  )!;
  assert.equal(advised.recommendation, 'admit_limited');
  assert.equal(advised.recommendationReason, 'Weight, not admissibility.');

  const ok = (await post(
    `/api/runs/${slug}/${id}/decide`,
    { gateId: 'G-01', optionId: 'admit' },
    root,
  ))!;
  assert.equal(ok.status, 200, JSON.stringify(ok.body));
  const res = ok.body as { ok: boolean; override: boolean; courtLine: string };
  assert.equal(res.ok, true);
  assert.equal(res.override, true, 'admit is not the recommendation');
  assert.match(res.courtLine, /So ordered/);
  const after = body<RunData>(`/api/runs/${slug}/${id}/court`);
  assert.equal(after.pending, null);
  const decided = after.gates.find((g) => g.id === 'G-01')?.decided;
  assert.ok(decided);
  assert.equal(decided.unadvised, false);
  assert.equal(
    (await post(`/api/runs/${slug}/${id}/decide`, { gateId: 'G-01', optionId: 'admit' }, root))!
      .status,
    409,
    'once',
  );
...
test('/stream sends a change after a file write and stops after close', async () => {
  ...
  // An assertion failure must still close the stream, or its watcher keeps the test runner alive.
  try {
    ...
    assert.deepEqual(JSON.parse(change.split('data: ')[1]!.trim()), {
      seq: LAST_SEQ,
      status: 'complete',
    });
